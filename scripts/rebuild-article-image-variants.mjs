/**
 * Rebuild responsive variants for article-learn-* and article-blog-* images.
 *
 * Usage: node scripts/rebuild-article-image-variants.mjs
 */
...
function getBreakpoints(width) {
  return BREAKPOINTS.filter((bp) => bp <= width)
    .concat(width)
...
async function main() {
  ...
  const targets = readdirSync(ROOT)
    .filter((file) => /^article-(learn|blog)-.+\.webp$/i.test(file))
    .map((file) => path.join(ROOT, file))

  console.log(`Processing ${targets.length} article images...`)
